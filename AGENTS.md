...
Version pairing: **jovian/sdl3 0.8.0 projects ext-sdl3 0.8.0**, which links SDL **3.4.4**. Neither published a stable 0.8.0 before this wave, so the package and extension versions now coincide; the composer constraint is `ext-sdl3: ^0.8.0`.

## Rules

1. **Projection, not composition.** A method is legitimate only if it is exactly one extension call with the same arguments in the same order. The test: *can it be written as one extension call?* If not, it belongs in `venusian-sdl3`. Both `tests/Generator/GenerateCheckTest.php` and `verify-parity.mjs` enforce this: every one of the 727 generated bodies must contain exactly one `Ext*::` call.
...
6. **`typedef enum` is `Enum|int`; a bitmask family stays `int`,** for parameters and returns alike, because PHP enums cannot be OR'd. This is jovian/metal's NS_ENUM / NS_OPTIONS split under SDL's spelling. The seven bitmask families are `SDLInitFlags`, `SDLWindowFlags`, `SDLKeymod`, `SDLBlendMode`, `SDLGPUShaderFormat`, `SDLGPUTextureUsageFlags`, `SDLGPUBufferUsageFlags`. Pass `SDLInitFlags::VIDEO->value | SDLInitFlags::EVENTS->value`. `SDLGPUBufferUsageFlags` is the one exception: it comes from the `extra-enums.php` mechanism (rule 10), not from a reachable parameter, so no projected signature is actually declared `int` against it — a createinfo `usage` value is passed as `SDLGPUBufferUsageFlags::VERTEX->value | SDLGPUBufferUsageFlags::INDEX->value` the same way.
...
10. **Constants are mined from the SDL3 headers, not invented.** ext-sdl3 0.8.0 exposes **zero** PHP-visible constants and zero functions — 31 classes and nothing else — so `/opt/homebrew/include/SDL3` is the authority. A family earns an enum by being *reachable*: it is the C type of a parameter or return on a projected method. `scripts/Generator/extra-enums.php` documents the exceptions — families with no reachable parameter or return, added because a downstream consumer genuinely needs the constant named as an enum rather than invented: `SDL_EventType` (the extension reads the event union apart in C; no prototype mentions the type), and, for `venusian-sdl3`'s SDL_GPU engine (task 16), sixteen `SDL_GPU*` families whose members live inside createinfo structs read apart the same way — `SDL_GPUPrimitiveType`, `SDL_GPULoadOp`, `SDL_GPUStoreOp`, `SDL_GPUVertexElementFormat`, `SDL_GPUVertexInputRate`, `SDL_GPUShaderStage`, `SDL_GPUFillMode`, `SDL_GPUCullMode`, `SDL_GPUFrontFace`, `SDL_GPUBlendOp`, `SDL_GPUBlendFactor`, `SDL_GPUFilter`, `SDL_GPUSamplerMipmapMode`, `SDL_GPUSamplerAddressMode`, `SDL_GPUTransferBufferUsage`, and the bitmask `SDL_GPUBufferUsageFlags`.
...
node scripts/gates/verify-parity.mjs          # PARITY_OK        727 = 727
node scripts/gates/verify-style.mjs           # STYLE_OK         86 files
node scripts/gates/verify-enum-values.mjs     # ENUM_VALUES_OK   925 cases vs cc
node scripts/gates/verify-ext-control.mjs     # EXT_CONTROL_CONSISTENT
vendor/bin/pest                               # 29 passed
